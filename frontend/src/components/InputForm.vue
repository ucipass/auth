...
export default {
  ...
  methods:{
    inputClick: function ( ev ) {
      if ( ev?.target?.type == "file") {
        ...
        store[payload.name].values[payload.id] = payload.value
      }
    },
    inputChange: function ( ev ) {
      const payload ={
          name: this.name,
          id: ev.target.name,
          value: ev?.target?.files?.length ? ev.target.files : ev.target.value     
      }
  ...
  mounted: function () {
    console.log("Mounted Input:", this.name ? this.name : "InputForm is not initiated with a name!")
  }
}
</script>
